...
def newJob(engine):
    ...
    select_stmt1 = "UPDATE dataset_en SET status = 1 WHERE sampleid IN (SELECT sampleid FROM dataset_intl TABLESAMPLE SYSTEM (0.05) WHERE status = 0 and language != 'en' LIMIT 10000 FOR UPDATE SKIP LOCKED) AND status = 0 RETURNING sampleid"
